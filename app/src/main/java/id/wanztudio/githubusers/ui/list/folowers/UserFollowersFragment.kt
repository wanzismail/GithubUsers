 ...
 */
class UserFollowersFragment : BaseFragment<UserDetailActivity>(), Injectable,
    UserListCallback {

    @Inject
    lateinit var viewModelFactory: ViewModelProvider.Factory

    private val userViewModel: UserFollowersViewModel by lazy {
    ...
    private fun getUserList(isPullToRefresh: Boolean) {
        ...
        userViewModel.getFollowersList(username)
            .observe(this) {
                when {
                    it.status.isLoading() -> {
                        if (isPullToRefresh) {
                    ...
                    it.status.isSuccessful() -> {
                        ...
                        it.data?.let { data ->
                           userList = DataMappingHelper.mapUserFollowerResponseToModel(data)
