...
import id.wanztudio.githubusers.ui.list.UserListAdapter
import id.wanztudio.githubusers.ui.list.UserListCallback
 ...
 */
class UserFollowingFragment : BaseFragment<UserDetailActivity>(), Injectable,
    UserListCallback {

    @Inject
    lateinit var viewModelFactory: ViewModelProvider.Factory

    private val userViewModel: UserFollowingViewModel by lazy {
    ...
    private fun getUserList(isPullToRefresh: Boolean) {
        ...
        userViewModel.getFollowingList(username)
            .observe(this) {
                when {
                    it.status.isLoading() -> {
                        if (isPullToRefresh) {
                    ...
                    it.status.isSuccessful() -> {
                        ...
                        it.data?.let { data ->
                            userList = DataMappingHelper.mapUserFollowingResponseToModel(data)
    ...
    companion object {
        fun newInstance(username: String): UserFollowingFragment {
